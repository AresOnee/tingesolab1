# ============================================
# SCRIPT DE DESPLIEGUE RAPIDO - TOOLRENT
...
try {
    ...
    Write-Error-Custom "Docker no esta corriendo. Por favor inicia Docker Desktop."
    exit 1
}

# Verificar que DOCKER_USERNAME este configurado
if (-not $env:DOCKER_USERNAME) {
    Write-Error-Custom "Variable DOCKER_USERNAME no esta configurada."
...
# ====== OPCION 1: BUILD LOCAL Y DESPLIEGUE ======
if ($Command -eq "build") {
    Write-Step "OPCION 1: BUILD LOCAL DE IMAGENES"
...
# ====== OPCION 2: PULL DESDE DOCKER HUB Y DESPLIEGUE ======
elseif ($Command -eq "pull") {
    Write-Step "OPCION 2: PULL DESDE DOCKER HUB"
...
# ====== OPCION 4: DETENER Y LIMPIAR TODO ======
elseif ($Command -eq "clean") {
    Write-Step "LIMPIEZA COMPLETA"

    Write-Warning-Custom "Esto eliminara todos los contenedores, volumenes e imagenes"
    $confirmation = Read-Host "Estas seguro? (S/N)"
...
# ====== OPCION POR DEFECTO: MOSTRAR AYUDA ======
else {
    Write-Host "============================================" -ForegroundColor Blue
    ...
    Write-Host "- Construir imagenes localmente"
    ...
    Write-Host "                    (requiere imagenes en Docker Hub)"
    Write-Host ""
    ...
    Write-Host "                    (elimina contenedores, volumenes e imagenes)"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "Acceso a la aplicacion despues del despliegue:" -ForegroundColor Yellow
